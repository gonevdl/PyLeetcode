        ...
        while True:
            ...
            for i in range(0, len(special)):
                ...
                if flag == True:
                    ...
                    dis = 0
                    for j in range(0, len(needs)):
                        dis += special[i][j] * price[j]
                    discount.append([i, dis - special[i][len(needs)]])
            if flag2 == False:
                break
            max = 0
            pos = 0
            for nums in discount:
                if nums[1] >= max:
                    max = nums[1]
                    pos = nums[0]
            total += special[pos][len(needs)]
            for i in range(0, len(needs)):
                needs[i] = needs[i] - special[pos][i]
        for i in range(0, len(needs)):
